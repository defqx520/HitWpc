package cn.edu.hit.ftcl.wearablepc.VoiceControl.speechUtils;
...
//				}
	...
	public static int myparseGrammarResult(String json) {
        int retID=-1;
		try {
			JSONTokener tokener = new JSONTokener(json);
			JSONObject joResult = new JSONObject(tokener);
           ...
           //处理每一个词
            for (int i = 0; i < words.length(); i++) {
                JSONObject wsItem = words.getJSONObject(i);   //一个词语对象
                JSONArray items = wsItem.getJSONArray("cw");  //只关注词对象的cw字段  （包括id和w）
                if ("<soft>".equals(wsItem.getString("slot"))) {     //如果这个词是soft中定义的词
                    // 可能会有多个联系人供选择，用中括号括起来，这些候选项具有相同的置信度
